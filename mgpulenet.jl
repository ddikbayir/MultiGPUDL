...
function train(w, data; lr=.1, epochs=3, iters=1800)
    ...
    for epoch=1:epochs
        for (x,y) in data
            #@show typeof(reshape(reshape(x,size(x,1)*size(x,2),size(x,4))[:,i*(size(x,4)/2)+1:(i+1)*size(x,4)/2],size(x,1),size(x,2),size(x,3),convert(Int,size(x,4)/2)))
           
            g = @parallel (+) for i = 0:1 # for each gpu give a subbatch
                gpu(i)
                lossgradient(w, reshape(reshape(x,size(x,1)*size(x,2),size(x,4))[:,i*convert(Int,size(x,4)/2)+1:(i+1)*convert(Int,size(x,4)/2)],size(x,1),size(x,2),size(x,3),convert(Int,size(x,4)/2)), y[i*convert(Int,size(y,1)/2)+1:(i+1)*convert(Int,size(y,1)/2)])
            end
...
function main(args=ARGS)
    ...
    if gpu() >= 0
        Knet.enableP2P() #enable gpu direct communication
    end
